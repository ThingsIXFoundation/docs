...
The globe will be divided into a number of hexes according to resolution 3 of the Uber-H3 system. The world is divided into 41162 of such hexes of that resolution.. For each of the hexes a coverage manager will be appointed. This manager is a smart-contract on the ICP blockchain in its separate canister.  The same coverage manager can be appointed for multiple hexes when scaling out to this number is not required yet.
Like owners of a Gateway can claim ownership of a Gateway, holders of a ThingsMapper can claim that they are holding one or more ThingsMappers to get the rewards.
By using the threshold-cryptography functionality of ICP, a ECDSA keypair can be generated and kept in the canister. This keypair in the ICP canister in turn is used to sign “payout” receipts. The “payout” receipts are combined in a single canister and then signed again. The resulting overall receipt is submitted to the smart-contract on the blockchain. This smart-contract will verify the signature of the ICP canister and if it’s valid, initiate the payout in ThingsIX tokens.
Maintenance Tokens
